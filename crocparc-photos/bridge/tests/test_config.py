...
def test_inbox_est_obligatoire(monkeypatch, tmp_path):
    monkeypatch.delenv("BRIDGE_INBOX_DIR", raising=False)
    with pytest.raises(ConfigError, match="BRIDGE_INBOX_DIR"):
...
def test_dotenv_ne_recouvre_pas_l_environnement(monkeypatch, tmp_path):
    ...
    env_file.write_text(
        "# commentaire\nBRIDGE_INBOX_DIR=./depuis-le-fichier\nWATERMARK_TEXT='CROC PARC'\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("BRIDGE_INBOX_DIR", str(tmp_path / "depuis-l-env"))
    config = Config.from_env(env_file=env_file, base_dir=tmp_path)
    assert config.inbox_dir == tmp_path / "depuis-l-env"
...
def test_chemins_relatifs_resolus_depuis_la_base(monkeypatch, tmp_path):
    monkeypatch.setenv("BRIDGE_INBOX_DIR", "./inbox")
    monkeypatch.setenv("BRIDGE_DATA_DIR", "./donnees")
    config = Config.from_env(base_dir=tmp_path)
    assert config.inbox_dir == (tmp_path / "inbox").resolve()
...
def test_ensure_dirs_cree_l_arborescence(config: Config):
    config.ensure_dirs()
    for directory in (config.inbox_dir, config.work_dir, config.archive_dir, config.output_dir, config.log_dir):
